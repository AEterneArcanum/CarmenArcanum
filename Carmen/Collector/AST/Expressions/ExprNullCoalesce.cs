...
namespace Arcane.Carmen.Collector.AST.Expressions
{
    ...
    {
        public ExprNullCoalesceParser()
            : base(ExpressionPriorities.NullCoalescing) { }
        ...
        public override bool TryParse(Token[] tokens, out Expression? expression)
        {
            ...
            if (!Expression.TryParse(tokens[..index], out var left) ||
                left is not ExprIdentifier && left is not ExprArrayAccess && left is not ExprMemberAccess && left is not ExprFunctionCall) return false; // Failed to parse left expression 
