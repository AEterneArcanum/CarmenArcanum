...
namespace Arcane.Carmen.Collector.AST.Expressions
{
    ...
    {
        public ExprFunctionParameterParser() :
            base(ExpressionPriorities.FunctionParameter) { }
        ...
        public override bool TryParse(Token[] tokens, out Expression? result)
        {
            ...
            if (!Expression.TryParse(tokens[1..idxAs], out var idExpr) ||
                idExpr is not ExprIdentifier || ((ExprIdentifier)idExpr).Type == IdentifierType.Variable) // Variable id accessed within the function.
                return false;
            if (isNullable) idxAs++;
            if (isPointer) idxAs++;
            if (isRestrict) idxAs++;
