...
namespace Arcane.Carmen.Collector.AST.Expressions
{
    ...
    {
        public ExprNullCheckParser()
            : base(ExpressionPriorities.NullCheck) { }
        ...
        public override bool TryParse(Token[] tokens, out Expression? expression)
        {
            ...
            if (!Expression.TryParse(tokens[..^1], out var expr) ||
                expr is not ExprIdentifier && expr is not ExprArrayAccess && expr is not ExprMemberAccess && expr is not ExprFunctionCall)
            {
