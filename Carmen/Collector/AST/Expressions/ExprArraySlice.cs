...
namespace Arcane.Carmen.Collector.AST.Expressions
{
    ...
    {
        public ExprArraySliceParser():base(ExpressionPriorities.ArraySlice) { }
        ...
        public override bool TryParse(Token[] tokens, out Expression? expression)
        {
            ...
            if (!unary)
            {
                ...
                if (!Expression.TryParse(tokens[(ofIndex + 1)..], out var array)) // can be member access identifier function call
                {
                    return false;
                }
